...
public interface StorageBlobManager {
  ...
  /**
   * 事务内 release：{@code ref_count - 1}，减到 0 时切换 DELETING 并在提交后唤醒后台回收。
   *
  ...
  /** 后台维护用：回收残留 DELETING blob 的 S3 对象并条件删除其行，返回处理行数。 */
  int sweepDeleting();
}
