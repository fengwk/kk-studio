...
/**
 * 目录浏览请求的应用级失败分类（platform gateway 计算）。
 *
 * <p>环境未知/不可用与超时是 gateway 在本地计算的结果 code。HTTP 映射：{@code ENVIRONMENT_NOT_FOUND}/{@code NOT_FOUND} →
 * 404，{@code ENVIRONMENT_UNAVAILABLE} → 409，{@code INVALID_PATH}/{@code NOT_DIRECTORY} → 400，{@code
 * TIMEOUT} → 504，{@code IO_ERROR} → 502。
 */
