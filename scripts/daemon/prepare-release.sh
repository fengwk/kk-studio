#!/usr/bin/env bash
# scripts/daemon/prepare-release.sh — stage the Environment Daemon release assets
#
# 单一职责：把一个已经构建好的 shaded Daemon JAR 与其法律文件暂存为一组可直接上传的
# 资产。脚本不上传任何东西、不打印任何凭证，也不触碰专用发布目录以外的路径。
#
# 用法：
#   scripts/daemon/prepare-release.sh <release-tag>
#
...
usage() {
  cat <<'EOF'
Usage: scripts/daemon/prepare-release.sh <release-tag>
