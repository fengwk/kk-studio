...
Covers the single asset-staging implementation (``scripts/release/prepare-daemon-release.sh``)
...
RELEASE_SCRIPT = REPOSITORY_ROOT / "scripts" / "release" / "prepare-daemon-release.sh"
...
def build_fixture_repository(root, jar_source):
    ...
    The script derives its repository root from its own location, so a copy under
    ``<root>/scripts/release`` exercises exactly the shipped implementation while the
    fixture JAR stands in for the Maven-built artifact.
    """
    root = Path(root)
    (root / "scripts" / "release").mkdir(parents=True)
    script = root / "scripts" / "release" / RELEASE_SCRIPT.name
...
class TestDaemonReleaseWorkflow(unittest.TestCase):
    ...
    def test_quality_gates_run_before_any_publication(self):
        gates = (
            ...
            "node --test scripts/e2e/tests/*.test.mjs",
            "node --test scripts/supply-chain/tests/*.test.mjs",
            '-Dproject.build.outputTimestamp="${{ steps.tag.outputs.source_date_epoch }}"',
            'scripts/release/prepare-daemon-release.sh "${GITHUB_REF_NAME}"',
        )
