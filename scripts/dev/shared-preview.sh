...
# 端口、日志目录与子命令都交给 scripts/dev/app.sh。
#
# 配置：默认读取 $HOME/.config/kk-studio/shared-preview.env，只允许用 SHARED_PREVIEW_ENV_FILE 指向别的路径。
# 该文件必须是 owner-only（无 group/other 权限位）的普通非符号链接文件，内容为占位符模板
# scripts/dev/shared-preview.env.example 列出的数据面 KEY=VALUE；解析规则见
# scripts/dev/app.sh。
...
if [ -z "$SHARED_PREVIEW_ENV_FILE" ]; then
  if [ -z "${HOME:-}" ]; then
    echo "ERROR: HOME must be set to resolve the default SHARED_PREVIEW_ENV_FILE" >&2
    exit 1
  fi
  SHARED_PREVIEW_ENV_FILE="$HOME/.config/kk-studio/shared-preview.env"
fi
