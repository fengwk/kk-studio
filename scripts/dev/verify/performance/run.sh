...
# scripts/dev/verify/performance/run.sh — free, machine-local performance baseline.
#
...
validate_report_root() {
  # 通过绝对 file URL 载入 runner，使校验不依赖调用者 cwd。
  node --input-type=module - "$REPORT_ROOT" "$REPO_ROOT" >/dev/null <<'NODE'
import path from 'node:path'
import { pathToFileURL } from 'node:url'

const [reportRoot, repoRoot] = process.argv.slice(2)
const runner = pathToFileURL(path.join(repoRoot, 'scripts/dev/verify/performance/runner.mjs')).href

try {
  const { validateReportRoot } = await import(runner)
  validateReportRoot(reportRoot, { repoRoot })
} catch (error) {
...
set +e
node "$SCRIPT_DIR/runner.mjs" "${RUNNER_ARGS[@]}" &
RUNNER_PID=$!
wait "$RUNNER_PID"
RUNNER_STATUS=$?
RUNNER_PID=
