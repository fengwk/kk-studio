...
RELIABILITY_MINIO_PORT=${RELIABILITY_MINIO_PORT:-19002}
...
export RELIABILITY_APP_PORT RELIABILITY_MINIO_PORT RELIABILITY_ENV_NAME
export RELIABILITY_REGISTRATION_TOKEN
...
usage() {
  cat <<'EOF'
Usage: scripts/dev/verify/reliability/stack.sh <command> [arguments]
...
environment_is_ready() {
  curl -fsS "$APP_URL/api/harness/environments" 2>/dev/null \
    | python3 -c '
import json
import sys
...
print_environment_status() {
  curl -fsS "$APP_URL/api/harness/environments" \
    | python3 -c '
import json
import sys
...
up_stack() {
  ...
  BACKEND_URL="$APP_URL" \
    ...
    python3 "$REPO_ROOT/scripts/dev/verify/reliability/sync_minimax_credentials.py" \
      --backend-url "$APP_URL"
}
...
' <"$REPO_ROOT/scripts/dev/verify/reliability/fixtures/NativeToolSmoke.java"
}
