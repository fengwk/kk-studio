...
#
# 该栈不读取宿主 MiniMax 凭据；真实 Provider 仍只能经 scripts/dev/verify/e2e/run.sh
...
# Compose 与运行时资产属于 deploy/；本脚本只做宿主生命周期编排。
COMPOSE_FILE="$REPO_ROOT/deploy/distributed/compose.yaml"
...
cmd_verify() {
  ...
  python3 "$SCRIPT_DIR/validate_distributed.py" --compose-file "$COMPOSE_FILE"
