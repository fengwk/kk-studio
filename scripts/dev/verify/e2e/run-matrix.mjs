 ...
 *
 * 结构：
 * - scripts/dev/verify/e2e/run-matrix.mjs   CLI + 报告
 * - scripts/dev/verify/e2e/lib/*            HTTP/fixtures/registry
 * - scripts/dev/verify/e2e/cases/*          用例注册
 *
 * 报告（gitignore）：
...
function writeSummaryAndReport(
  ...
  if (failed.length) {
    ...
    lines.push('4. Re-run: `scripts/dev/verify/e2e/run.sh --only <caseId>`')
    lines.push('')
  } else {
    lines.push('## Notes')
    lines.push('')
