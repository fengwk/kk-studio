#!/usr/bin/env bash
# scripts/dev/verify/e2e/run.sh — kk-studio 矩阵化 E2E 入口
#
# 快速开始：
#   scripts/dev/verify/e2e/run.sh                  # 复用已启动服务，跑 L1（免费）
#   scripts/dev/verify/e2e/run.sh --rebuild        # 强制 Java 21 clean package 并重启 backend/frontend
#   scripts/dev/verify/e2e/run.sh --real           # + 真实四 Provider 轮次（需 8 个 TEST_* 变量）
#   scripts/dev/verify/e2e/run.sh --real --with-branch
#   scripts/dev/verify/e2e/run.sh --real --with-tools --with-canvas-storage  # 真实 Tool turn（Resource 外部化需 S3）
#   scripts/dev/verify/e2e/run.sh --distributed    # + 双节点 distributed mock topology（免费）
#   scripts/dev/verify/e2e/run.sh --ui              # + Playwright UI E2E（截图进报告）
#   scripts/dev/verify/e2e/run.sh --list           # 只打印矩阵，不执行
#
# 文档事实源：
...
if [ "$LIST_ONLY" = "true" ]; then
  node "$SCRIPT_DIR/run-matrix.mjs" --list
  exit 0
fi
if [ "$DOCS_ONLY" = "true" ]; then
  node "$SCRIPT_DIR/run-matrix.mjs" --docs
  exit 0
fi
...
DISTRIBUTED_RUNNER="$SCRIPT_DIR/distributed.sh"
...
step "Running matrix"
set +e
node "$SCRIPT_DIR/run-matrix.mjs" "${MATRIX_ARGS[@]}"
MATRIX_RC=$?
set -e
