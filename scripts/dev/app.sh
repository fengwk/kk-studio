...
# 本机 preview 的外部数据面配置：只有调用方显式给出 SHARED_PREVIEW_ENV_FILE 时才读取（见 usage），
# 未给出时本脚本保持通用/e2e 行为，不接触任何外部配置文件。
SHARED_PREVIEW_ENV_FILE=${SHARED_PREVIEW_ENV_FILE:-}
# 配置文件允许出现的键。整份白名单之外的名字一律视为配置错误，避免把任意环境变量注入
# 长驻 Backend/Vite 进程。
SHARED_PREVIEW_ENV_REQUIRED_KEYS=(
  KK_STUDIO_DB_URL
  KK_STUDIO_DB_USER
...
SHARED_PREVIEW_ENV_OPTIONAL_KEYS=(
...
shared_preview_env_key_allowed() {
  local key=$1 candidate
  for candidate in "${SHARED_PREVIEW_ENV_REQUIRED_KEYS[@]}" "${SHARED_PREVIEW_ENV_OPTIONAL_KEYS[@]}"; do
...
load_shared_preview_env_file() {
  local file=$1
  local line_number=0 line trimmed key value candidate
  local owner mode
  case "$file" in
    /*) ;;
    *) fail "SHARED_PREVIEW_ENV_FILE must be an absolute path" ;;
  esac
  if [ ! -f "$file" ]; then
    fail "SHARED_PREVIEW_ENV_FILE must be an existing regular file"
  fi
  if [ -L "$file" ]; then
    fail "SHARED_PREVIEW_ENV_FILE must not be a symbolic link"
  fi
  owner=$(file_owner_uid "$file") || fail "cannot read the SHARED_PREVIEW_ENV_FILE owner"
  if [ "$owner" != "$(id -u)" ]; then
    fail "SHARED_PREVIEW_ENV_FILE must be owned by the current user"
  fi
  mode=$(file_mode "$file") || fail "cannot read the SHARED_PREVIEW_ENV_FILE permissions"
  case "$mode" in
    *[!0-7]*) fail "cannot interpret the SHARED_PREVIEW_ENV_FILE permissions" ;;
  esac
  if (( (8#$mode & 8#077) != 0 )); then
    fail "SHARED_PREVIEW_ENV_FILE must not grant group or other permissions"
  fi
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    case "$trimmed" in
      *=*) ;;
      *) fail "malformed SHARED_PREVIEW_ENV_FILE line $line_number: expected KEY=VALUE" ;;
    esac
    key=${trimmed%%=*}
    value=${trimmed#*=}
    if ! shared_preview_env_key_allowed "$key"; then
      fail "unknown key on SHARED_PREVIEW_ENV_FILE line $line_number: $key"
    fi
    printf -v "$key" '%s' "$value"
  done < "$file"
  step "Loaded external data-plane settings from SHARED_PREVIEW_ENV_FILE (values not printed)"
}

if [ -n "$SHARED_PREVIEW_ENV_FILE" ]; then
  load_shared_preview_env_file "$SHARED_PREVIEW_ENV_FILE"
fi
...
# scripts/dev/shared-preview.sh 用该标记声明「本次运行必须连外部数据面」：此时 profile、Flyway 与
# Harness worker 开关都必须与 NAS 上唯一 Flyway owner / Harness worker 的取值一致。
KK_STUDIO_SHARED_PREVIEW=${KK_STUDIO_SHARED_PREVIEW:-false}

# 外部数据面 preview 的 fail-closed 校验：配置缺失、profile 不是 prod，或本机进程试图
# 承担 Flyway / 分布式 Work，都在启动任何服务之前失败。
require_external_data_plane() {
  local key
  if [ -z "$SHARED_PREVIEW_ENV_FILE" ]; then
    fail "KK_STUDIO_SHARED_PREVIEW=true requires SHARED_PREVIEW_ENV_FILE"
  fi
  for key in "${SHARED_PREVIEW_ENV_REQUIRED_KEYS[@]}"; do
    if [ -z "${!key:-}" ]; then
      fail "SHARED_PREVIEW_ENV_FILE must define a value for $key"
    fi
  done
...
if [ "$KK_STUDIO_SHARED_PREVIEW" = "true" ]; then
  require_external_data_plane
fi
...
BACKEND_JAR="$REPO_ROOT/web/target/kk-studio-web-1.0.0.jar"
...
Environment:
  ...
  # scripts/dev/shared-preview.sh 会额外设置 KK_STUDIO_SHARED_PREVIEW=true，此时缺配置或
...
set_loaded_shared_preview_env_exported() {
  local exported=$1 key
  if [ -z "$SHARED_PREVIEW_ENV_FILE" ]; then
    return
  fi
  for key in "${SHARED_PREVIEW_ENV_REQUIRED_KEYS[@]}" "${SHARED_PREVIEW_ENV_OPTIONAL_KEYS[@]}"; do
    if [[ ! -v $key ]]; then
      continue
    fi
