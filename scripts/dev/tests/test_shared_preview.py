"""Permanent contracts for the packaged app image, the shared preview and its config loader."""
...
DEV_SCRIPT = REPOSITORY_ROOT / "scripts" / "dev" / "app.sh"
SHARED_PREVIEW_SCRIPT = REPOSITORY_ROOT / "scripts" / "dev" / "shared-preview.sh"
SHARED_PREVIEW_TEMPLATE = REPOSITORY_ROOT / "scripts" / "dev" / "shared-preview.env.example"
PUBLISH_WORKFLOW = REPOSITORY_ROOT / ".github" / "workflows" / "docker-publish.yml"

# 外部数据面凭据：PREVIEW_MARKER 未设置时不校验完整性，标记生效后缺一不可。
REQUIRED_KEYS = (
    "KK_STUDIO_DB_URL",
    "KK_STUDIO_DB_USER",
...
PREVIEW_MARKER = "KK_STUDIO_SHARED_PREVIEW"
...
def placeholder_config(omit=(), empty=(), optional=False):
    ...
    lines = ["# kk-studio shared-preview fixture", ""]
...
def write_config(directory, content, name="shared-preview.env", mode=0o600):
...
def clean_environment():
    """Return an environment without any inherited preview or data-plane input."""
    environment = dict(os.environ)
    for name in (
        "SHARED_PREVIEW_ENV_FILE",
        "KK_STUDIO_REPO_ROOT",
        PREVIEW_MARKER,
...
def source_dev_script(snippet, overrides=None):
    """Source the real dev app script and run one snippet against its parsed configuration."""
...
def run_shared_preview(arguments, overrides=None):
    """Run the shared preview entry point with only the inputs a caller is allowed to provide."""
    environment = {
    ...
    environment.update(overrides or {})
    return subprocess.run(
        ["bash", str(SHARED_PREVIEW_SCRIPT), *arguments],
        cwd=REPOSITORY_ROOT,
        env=environment,
        text=True,
        capture_output=True,
...
class TestSharedPreviewEntryPoint(unittest.TestCase):
    """scripts/dev/shared-preview.sh is the one-command shared preview over the packaged Backend."""

    def test_shared_preview_forces_prod_profile_without_flyway_or_harness_workers(self):
        """The shared preview must never own migrations or distributed Harness work."""
        entry_point = SHARED_PREVIEW_SCRIPT.read_text()

        for forced in (
            "export SPRING_PROFILES_ACTIVE=prod",
            "export SPRING_FLYWAY_ENABLED=false",
            "export KK_STUDIO_HARNESS_RUNTIME_WORKERS_ENABLED=false",
            f"export {PREVIEW_MARKER}=true",
        ):
            self.assertIn(forced, entry_point)
        self.assertIn('exec "$SCRIPT_HOME/app.sh" "$@"', entry_point)

    def test_shared_preview_uses_the_owner_only_home_config_unless_overridden(self):
        """The default config path is under HOME; SHARED_PREVIEW_ENV_FILE is the only override."""
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = run_shared_preview(["status"], {"HOME": str(home)})
            self.assertEqual(0, result.returncode, result.stdout + result.stderr)
            self.assertIn("backend: stopped", result.stdout)
            # 入口不设置监听地址：Backend/Vite 仍由 scripts/dev/app.sh 默认绑定 loopback。
            self.assertIn("url=http://127.0.0.1:18080", result.stdout)
            self.assertIn("url=http://127.0.0.1:5173", result.stdout)

            (config_dir / "shared-preview.env").unlink()
            override_dir = Path(temporary) / "elsewhere"
            override_dir.mkdir()
            override = write_config(override_dir, placeholder_config())
            result = run_shared_preview(["status"], {"HOME": str(home), "SHARED_PREVIEW_ENV_FILE": str(override)})
            self.assertEqual(0, result.returncode, result.stdout + result.stderr)

    def test_shared_preview_overrides_conflicting_caller_switches_but_keeps_listen_overrides(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = run_shared_preview(
                ["status"],
                {
                    "SHARED_PREVIEW_ENV_FILE": str(config),
    ...
    def test_shared_preview_fails_closed_without_a_config_file(self):
        """A missing config must stop the preview instead of starting a prod app without data."""
        with tempfile.TemporaryDirectory() as temporary:
            result = run_shared_preview(["status"], {"HOME": temporary})
            self.assertNotEqual(0, result.returncode)
            self.assertIn("SHARED_PREVIEW_ENV_FILE", result.stderr)

    def test_shared_preview_refuses_relaxed_config_permissions_without_echoing_values(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = run_shared_preview(["status"], {"SHARED_PREVIEW_ENV_FILE": str(config)})
            self.assertNotEqual(0, result.returncode)
            self.assertIn("group or other permissions", result.stderr)
            self.assertNotIn(VALUE_PREFIX, result.stdout + result.stderr)

    def test_shared_preview_help_and_empty_invocation_describe_the_forwarded_commands(self):
        """`--help` documents the forwarded lifecycle and the config override; no args fails."""
        help_result = run_shared_preview(["--help"])
        ...
        self.assertIn("SHARED_PREVIEW_ENV_FILE", help_result.stdout)

        empty_result = run_shared_preview([])
        self.assertNotEqual(0, empty_result.returncode)
        self.assertIn("Usage:", empty_result.stdout + empty_result.stderr)


class TestSharedPreviewConfigLoader(unittest.TestCase):
    """scripts/dev/app.sh loads the external data-plane file only when asked, and fail closed."""

    def test_generic_use_without_a_config_file_is_unchanged(self):
        """Without SHARED_PREVIEW_ENV_FILE the script keeps its own e2e defaults and reads no file."""
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = source_dev_script(
                (
                    ...
                    "set_loaded_shared_preview_env_exported true\n"
                    f'bash -c \'printf "CHILD:%s\\\\n" "${password_key}"\'\n'
                    "set_loaded_shared_preview_env_exported false\n"
    ...
    def test_start_exports_data_plane_only_around_backend_fork(self):
        ...
        self.assertIn("set_loaded_shared_preview_env_exported true", backend_handoff)
        self.assertIn('run_detached "$log_file" "$@"', backend_handoff)
        self.assertIn("set_loaded_shared_preview_env_exported false", backend_handoff)
    ...
    def test_file_requirements_are_enforced_before_parsing(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            relative = subprocess.run(
                ["bash", "-c", f"source {shlex.quote(str(DEV_SCRIPT))}\n:"],
                cwd=root,
                env={**clean_environment(), "SHARED_PREVIEW_ENV_FILE": "shared-preview.env"},
                text=True,
                capture_output=True,
                check=False,
            )
            ...
            missing = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(root / "missing.env")})
            self.assertNotEqual(0, missing.returncode)
            self.assertIn("existing regular file", missing.stderr)

            directory = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(root)})
            ...
            result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(symlink)})
            self.assertNotEqual(0, result.returncode)
            self.assertIn("symbolic link", result.stderr)

            for mode in (0o640, 0o604, 0o666):
                valid.chmod(mode)
                result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(valid)})
    ...
    def test_malformed_and_unknown_keys_fail_without_echoing_the_line(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(malformed)})
            self.assertNotEqual(0, result.returncode)
            self.assertIn("malformed SHARED_PREVIEW_ENV_FILE line 2", result.stderr)
            ...
            result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(unknown)})
            self.assertNotEqual(0, result.returncode)
            self.assertIn("unknown key on SHARED_PREVIEW_ENV_FILE line 1: KK_STUDIO_NOT_ALLOWED", result.stderr)
            self.assertNotIn(unknown_value, result.stdout + result.stderr)

    def test_preview_marker_requires_complete_prod_configuration(self):
        """Under the preview marker, missing data or a foreign execution owner fails closed."""
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            good = write_config(root, placeholder_config(), name="good.env")
            marker = {PREVIEW_MARKER: "true", "SPRING_PROFILES_ACTIVE": "prod",
                      "SPRING_FLYWAY_ENABLED": "false",
                      "KK_STUDIO_HARNESS_RUNTIME_WORKERS_ENABLED": "false"}

            result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(good), **marker})
            self.assertEqual(0, result.returncode, result.stdout + result.stderr)

            no_file = source_dev_script(":", marker)
            self.assertNotEqual(0, no_file.returncode)
            self.assertIn("requires SHARED_PREVIEW_ENV_FILE", no_file.stderr)
            ...
            result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(omitted), **marker})
            self.assertNotEqual(0, result.returncode)
            ...
            result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(emptied), **marker})
            self.assertNotEqual(0, result.returncode)
            ...
            result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(no_database), **marker})
            self.assertNotEqual(0, result.returncode)
            ...
            for override, expected in (
                ...
                overrides = {"SHARED_PREVIEW_ENV_FILE": str(good), **marker}
            ...
            result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(good), **workers_unset})
            self.assertNotEqual(0, result.returncode)
    ...
    def test_config_values_are_never_printed_on_success_or_failure(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            scenarios = [
                {"SHARED_PREVIEW_ENV_FILE": str(good)},
                {"SHARED_PREVIEW_ENV_FILE": str(good), PREVIEW_MARKER: "true",
                 "SPRING_PROFILES_ACTIVE": "prod", "SPRING_FLYWAY_ENABLED": "false",
                 "KK_STUDIO_HARNESS_RUNTIME_WORKERS_ENABLED": "false"},
                {"SHARED_PREVIEW_ENV_FILE": str(root / "missing.env"), PREVIEW_MARKER: "true",
                 "SPRING_PROFILES_ACTIVE": "prod", "SPRING_FLYWAY_ENABLED": "false",
                 "KK_STUDIO_HARNESS_RUNTIME_WORKERS_ENABLED": "false"},
                {"SHARED_PREVIEW_ENV_FILE": str(good), "SPRING_PROFILES_ACTIVE": "e2e",
                 "SPRING_FLYWAY_ENABLED": "true"},
            ]
            ...
            result = source_dev_script(":", {"SHARED_PREVIEW_ENV_FILE": str(readable)})
            self.assertNotIn(VALUE_PREFIX, result.stdout + result.stderr)
    ...
    def test_template_is_placeholder_only_and_points_at_the_owner_only_path(self):
        ...
        template = SHARED_PREVIEW_TEMPLATE.read_text()

        for key in REQUIRED_KEYS + OPTIONAL_KEYS:
            self.assertRegex(template, rf"(?m)^{key}=$", f"{key} must be an empty placeholder")
        self.assertIn(".config/kk-studio/shared-preview.env", template)
        self.assertIn("SHARED_PREVIEW_ENV_FILE", template)
...
class TestDevScriptArtifactContracts(unittest.TestCase):
    """Generic scripts/dev/app.sh behavior that the shared preview still depends on."""
    ...
    def test_dev_script_rebuilds_the_backend_when_the_stamp_is_stale(self):
        ...
        self.assertIn('git -C "$REPO_ROOT" rev-parse HEAD', revision_fn)
    ...
    def test_ensure_frontend_deps_reinstalls_only_when_the_lock_changed(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            app_home = root / "app"
            (app_home / "scripts" / "dev").mkdir(parents=True)
            # 仓库根由 worktree 标记向上解析，因此夹具必须是一个可识别的 checkout。
            (app_home / ".git").mkdir()
            shutil.copy(DEV_SCRIPT, app_home / "scripts" / "dev" / "app.sh")
            ...
            def run_ensure(overrides=None):
                ...
                environment.update(overrides or {})
                return subprocess.run(
                    ["bash", "-c", "source scripts/dev/app.sh\nensure_frontend_deps"],
                    cwd=app_home,
                    env=environment,
                    text=True,
                    capture_output=True,
...
class TestRemovedDevTopology(unittest.TestCase):
    ...
    ALLOWED_SPELLING_FILES = {
        # 本文件持有这些标识符本身。
        "scripts/dev/tests/test_shared_preview.py",
    }
    REMOVED_DAEMON_FLAG_ALLOWED = {
        ...
        "scripts/dev/tests/test_shared_preview.py",
        "scripts/dev/verify/e2e/tests/test_daemon_bootstrap_contracts.py",
        "scripts/dev/verify/reliability/tests/test_daemon_entrypoint_contracts.py",
    }
