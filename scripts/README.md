...
| 导出 Agent catalog（Provider / Model / Agent 定义） | [ops/export-agent-catalog.sh](ops/export-agent-catalog.sh) | 原生 libpq 客户端（`psql`）、python3、CLI 或 `VPS_POSTGRES_*`/libpq 连接设置 | 只读目标库；在仓库外 owner-only 目录写入版本化包（`catalog.sql`、`manifest.json`、`sha256sums.txt`，权限 0600） | 否 |
