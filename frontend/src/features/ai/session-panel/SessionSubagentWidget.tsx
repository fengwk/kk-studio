...
export function SessionSubagentWidget({ taskTree }: { taskTree: SubagentTaskNode[] }) {
  const running = flattenRunning(taskTree)
  if (running.length === 0) {
    return null
  }
  return (
    <div className="session-subagent-widget" aria-label="运行中的子代理">
      <div className="session-subagent-widget-title">
        ⟳ subagents running (
        {running.length}
        )
      </div>
      <ul>
        {running.map((item) => (
          <li key={item.id}>
            <span className="session-subagent-prefix">{item.prefix}</span>
            <span className="session-subagent-name">{item.name}</span>
            <span className="session-subagent-meta">
              {item.status}
