...
public class StudioProjectErrorAdvice {
  ...
  public ResponseEntity<Result<Void>> handleHarnessConflict(HarnessRuntimeConflictException error) {
    Map<String, Object> ctx = new LinkedHashMap<>();
    ctx.put("detail", "Coordinator runtime conflict occurred");
    return build(
        HttpStatus.CONFLICT,
        "PROJECT_RUNTIME_CONFLICT",
        "Coordinator runtime conflict occurred",
        ctx);
  }
  ...
  public ResponseEntity<Result<Void>> handleHarnessNotFound(HarnessRuntimeNotFoundException error) {
    Map<String, Object> ctx = new LinkedHashMap<>();
    ctx.put("detail", "Coordinator runtime resource not found");
    return build(
        HttpStatus.NOT_FOUND,
        "PROJECT_RUNTIME_NOT_FOUND",
        "Coordinator runtime resource not found",
        ctx);
  }
