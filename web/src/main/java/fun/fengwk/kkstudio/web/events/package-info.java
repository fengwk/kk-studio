 ...
 * {"version":1,"type":"event","resource":{"kind":"thread","id":"<canonical UUID>"},"name":"revision","cursor":"<canonical decimal>","data":{"revision":"<canonical decimal>"}}
 ...
 * 该 cursor 之后的事件保证送达，之前的由客户端随后拉取的 snapshot/changes 覆盖。revision/version 事件的 {@code data} 分别为 {@code
 * {"revision":"N"}} / {@code {"version":"N"}}，顶层 {@code cursor} 必带且与之相等；realtime 事件的 {@code data} 为
 * realtime codec JSON 对象且不携带 {@code cursor}。事件帧在 ack 帧之后才发送。非法帧（ {@code INVALID_FRAME}）或发送过载（{@code
 * BACKPRESSURE}）时发送连接级 error 帧并关闭连接；未知资源只回资源级 {@code RESOURCE_NOT_FOUND} 并保持连接，客户端重连恢复。
 */
