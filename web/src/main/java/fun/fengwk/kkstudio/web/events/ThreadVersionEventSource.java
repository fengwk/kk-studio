...
/** Thread durable version 通知的最小边界：订阅原子返回建立瞬间的 cursor，之后的事件保证送达。 */
interface ThreadVersionEventSource {

  record Event(String version, boolean resync) {
    public Event {
      if (resync) {
        if (version != null) {
          throw new IllegalArgumentException("resync event must not carry a version");
        }
      } else if (version == null || !version.matches("0|[1-9]\\d*")) {
        throw new IllegalArgumentException(
            "version event must carry a canonical non-negative decimal version");
      } else {
        try {
          Long.parseLong(version);
        } catch (NumberFormatException error) {
          throw new IllegalArgumentException("version event exceeds bigint range", error);
        }
      }
    }
  }
