...
final class GoalToolSupport {
  ...
  static Optional<GoalState> latest(BranchView branch) {
    return branch.latestCustomEntry(GoalPlugin.ID, GoalPlugin.STATE_TYPE).map(CODEC::decode);
  }
  ...
  static PluginToolResult stateChange(ToolCall call, GoalState state) {
  ...
  static PluginToolResult success(ToolCall call, String text) {
    return PluginToolResult.withoutIntents(success(call.id(), text));
  }

  static PluginToolResult error(ToolCall call, RuntimeException error) {
    String message = error.getMessage();
    return PluginToolResult.withoutIntents(
        ToolResult.error(
            call.id(),
