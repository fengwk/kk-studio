...
abstract class AbstractProviderTestSupport {
     ...
     *
     * 约定：
     * - 必须与 ProviderTestFixtures 中 contract fixture 的 providerName 对齐。
     */
     ...
     *
     * 当变量缺失时，使用 assume 让对应 live test 自动跳过。
     */
    protected ProviderInfo providerInfo() {
        String baseUrl = System.getenv(baseUrlEnvName());
        String apiKey = System.getenv(apiKeyEnvName());
        Assumptions.assumeTrue(baseUrl != null && !baseUrl.isBlank(), () -> "missing env: " + baseUrlEnvName());
        Assumptions.assumeTrue(apiKey != null && !apiKey.isBlank(), () -> "missing env: " + apiKeyEnvName());
     ...
     *
     * 该方法主要供 HTTP contract test 使用，以便把 provider 指向本地探针服务。
     */
     ...
     *
     * 约定：
     * - 使用固定 temperature，降低 live test 波动。
     * - 只保留最小必要参数，避免把测试与无关请求参数耦合。
     */
     ...
     *
     * 这份工具描述同时用于：
     * - live test 的真实 tool-call 场景
     * - contract test 的 request schema 校验
     */
    protected ToolInfo echoTool() {
        return ToolInfo.builder()
            ...
            .inputSchema(ToolParamsSchema.builder()
                ...
                .properties(Map.of("text", ToolStringSchema.builder().description("text to echo").build()))
    ...
    protected void assertCommonMetadata(AssistantMetadata metadata) {
        ...
        assertNotNull(metadata.getModelName(), () -> providerName() + " metadata.modelName must not be null");
    }
