...
public class ModelRetryConfigTest {
    ...
    public void testReturnsZeroWhenRetryCountOrBaseDelayDisablesBackoff() {
        assertEquals(Duration.ZERO, ModelRetryConfig.builder().baseDelay(Duration.ofMillis(10)).build().nextDelay(0));
        assertEquals(Duration.ZERO, ModelRetryConfig.builder().build().nextDelay(1));
        assertEquals(Duration.ZERO, ModelRetryConfig.builder().baseDelay(Duration.ZERO).build().nextDelay(1));
    }
    ...
    public void testExponentialBackoffIsCappedByMaxDelay() {
        ModelRetryConfig config = ModelRetryConfig.builder()
