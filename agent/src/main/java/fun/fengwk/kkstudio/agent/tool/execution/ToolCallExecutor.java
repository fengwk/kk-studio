...
public class ToolCallExecutor {
    ...
    public ToolExecutionHandle execute(ToolRegistration registration,
        ...
        if (timeoutSeconds < 0) {
            throw new IllegalArgumentException("tool timeoutSeconds must not be negative: " + registration.getName());
        }
        ...
        GuardedToolExecutionHandler guardedHandler = new GuardedToolExecutionHandler(
            request,
            listener,
            managedHandle,
            () -> cancelTimeout(timeoutTaskRef));
        managedHandle.setCancelHook(guardedHandler::onRuntimeCancel);
        if (timeoutSeconds > 0) {
            ScheduledTask timeoutTask = agentScheduler.schedule(
                Duration.ofSeconds(timeoutSeconds),
                () -> guardedHandler.onTimeout(timeoutSeconds));
        ...
        try {
            Future<?> future = executorService.submit(() -> executeTool(tool, request, guardedHandler, managedHandle));
