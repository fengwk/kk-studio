 ...
 *
 * <p>实际工作：在内部构建 {@link AgentSessionWriter} / {@link AgentAssistantRunner} / {@link
 * AgentToolOrchestrator} 三个协作者，使用注入的 {@link AgentRuntimeConfigResolver}， 然后构造 {@link Agent}。
 *
 * @author fengwk
 */
public class AgentFactory {
  ...
  public Agent load(
    ...
    AgentToolOrchestrator tools =
        new AgentToolOrchestrator(
            writer, deps.toolCallExecutor(), holder.enqueueSignal, holder.continueAssistant);

    AgentAssistantRunner assistant =
        new AgentAssistantRunner(writer, holder.enqueueSignal, holder.startToolBatch);
  ...
  /** 解决 Agent → assistant → Agent 的循环引用：先以 lambda 形式持有 Agent 引用， Agent 构造完后再注入。 */
  private static final class AgentToAssistants {

    private Agent agent;

    final Consumer<AgentSignal> enqueueSignal = signal -> agent.enqueueSignal(signal);

    final Consumer<List<ToolCall>> startToolBatch = toolCalls -> agent.startToolBatch(toolCalls);

    final Runnable continueAssistant = () -> agent.continueAssistantFromToolBatch();
