 ...
 * - Provider 对外只暴露一个稳定入口：asyncChat(...)
     * - Agent 侧只关心 AgentMessage、ModelInfo、Variant、ToolInfo 与流式回调，不关心具体 SDK 类型
...
public interface Provider {
     ...
     *
     * 调用约束：
     * - messages 是本轮重放后的完整上下文
     * - modelInfo 提供模型稳定信息
     * - variant 提供本次实际请求参数
     * - toolInfos 提供本次允许调用的工具描述
     * - handler 负责接收流式 text/thinking/toolcall/complete/error 回调
     */
    AssistantResponseHandle asyncChat(List<AgentMessage> messages,
                                       ModelInfo modelInfo,
                                       Variant variant,
