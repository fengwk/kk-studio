...
class GuardedToolExecutionHandler implements ToolExecutionHandler {
    ...
    GuardedToolExecutionHandler(ToolCallRequest request,
    ...
    void onTimeout(long timeoutSeconds) {
        fail(new ToolTimeoutException(request.getToolCallId(), request.getToolName(), timeoutSeconds), true);
    }
    ...
    private void warn(String reason) {
        log.warn("[tool-execution] ignore tool callback: toolCallId={} toolName={} reason={}",
            request.getToolCallId(), request.getToolName(), reason);
    }
