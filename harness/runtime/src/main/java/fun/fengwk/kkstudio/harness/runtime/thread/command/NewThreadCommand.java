 ...
 * 不同。
 *
 * <p>因此三参构造可用于承载「payload 已物化、但幂等键（{@code idempotencyKey} + {@code requestHash}）保持原始值」的命令； preflight
 * 应使用 {@link #withPayload} 替换 durable payload，以确保同 idempotencyKey 的 raw 重试仍能命中 ordered replay。下游
 * platform 的 attachment 物化逻辑应使用该方法。
 */
public record NewThreadCommand(
    ThreadCommandPayload payload, UUID idempotencyKey, String requestHash) {

  /** 由服务端按 raw payload 计算 request hash（两参便捷构造，仅用于 raw 请求形态的命令）。 */
  public NewThreadCommand(ThreadCommandPayload payload, UUID idempotencyKey) {
    this(
        payload,
        idempotencyKey,
  ...
  /**
   * 用 durable payload 重建命令，保留原始 {@code idempotencyKey} 与 {@code requestHash}。
   *
   * <p>preflight 或应用 use-case 把瞬时 ATTACHMENT 物化为 durable RESOURCE 后调用：结果命令的 payload 是 durable
   * 形态，但幂等键保持客户端原始值，因此携带原始 ATTACHMENT 的请求重试仍能按 idempotencyKey + requestHash 命中 ordered replay。
   */
  public NewThreadCommand withPayload(ThreadCommandPayload durablePayload) {
    ...
    return new NewThreadCommand(durablePayload, idempotencyKey, requestHash);
  }
}
