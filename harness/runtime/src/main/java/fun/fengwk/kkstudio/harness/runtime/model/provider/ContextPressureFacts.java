 ...
 * providerErrorCodeOrType}、{@code errorMessage}，由 platform 的 extractor 从异常链抽取）与生成响应（{@code
 * stopReason}、 {@code usage}、{@code contextWindow}）。除 {@code providerType} 外所有字段可为空；{@code
 * contextWindow} 非空时 必须为正整数。该快照不持久化，也不携带 {@link ProviderErrorKind} 等 durable 状态。
 */
