...
public interface RunControlMessageStore {
  ...
  /**
   * 拉取指定 original run + kind 的 PENDING 行；按 id asc。 调用方负责持有 Run/Session 锁，避免与 CAS 推进竞争； 若
   * originalRunId 为 null（如 FOLLOW_UP 尚未挂到 run），查询结果为空。
   */
  ...
  /**
   * CAS：仅当行仍为 PENDING 时推进为 CONSUMED；返回 true 表示推进成功。 consumedRunId/consumedEntryId 决定后不可变；consumedAt
   * 由实现根据 now 写入。
   */
  boolean markConsumed(long controlId, long consumedRunId, long consumedEntryId, Instant now);

  /**
   * CAS：仅当行仍为 PENDING 时推进为 PROMOTED；调用方传入新创建或即将创建的目标 run id （FOLLOW_UP 直接 promotion 时即新 run 的 id）。
   */
  ...
  /** 批量清空指定 session 的所有 PENDING（含 run_id is null 的行）；返回受影响行数。 */
  int clearPendingBySession(long sessionId, Instant now);
}
