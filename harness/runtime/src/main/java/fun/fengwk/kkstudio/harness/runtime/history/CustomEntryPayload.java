...
/**
 * 业务插件追加的透明 branch state Entry：不参与 turn grammar（允许 ROOT 后任意位置，包括 open/closed turn）， provider
 * 消息投影默认忽略它。字段：canonical 小写 dotted/dashed {@code pluginId} / {@code customType}、 正数 {@code
 * schemaVersion} 与 bounded canonical JSON object 字符串 {@code dataJson}。
 */
public record CustomEntryPayload(
    String pluginId, String customType, int schemaVersion, String dataJson)
