 ...
 * fallback 对照 preparation 的 executionModel）。YOLO 不进入 spec，也不参与校验。
 *
 * <p>压缩 turn 必须零 binding，model/variant 等于 {@code preparation.executionModel()}，且 Resolved 的
 * model/variant 必须匹配 executionModel；实际 contextWindow / maxOutputTokens 由该 execution model 的
 * Resolver 结果冻结，不能与 primary model 的规划值比较。正常 turn 的 basis path 末尾不得带 COMPACTION TURN_START。任何不一致都是
