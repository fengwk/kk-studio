 ...
 * settings 快照，MESSAGE / CUSTOM_MESSAGE / ASSISTANT_ERROR / ASSISTANT_ABORTED 保存对话事实，CUSTOM 保存
 * 业务插件追加的透明 branch state（不参与 turn grammar、默认不投影），TURN_END 保存关闭结果与 continuation obligation。EntryPath
 * 校验 root-to-head 链的同 Session、parent 连续、turn open/close 与 createdAt 不变量； baseSettings 沿 path 推导最近的
 * ROOT/TURN_START 完整 snapshot。类型引用优先复用已存在的 {@link
