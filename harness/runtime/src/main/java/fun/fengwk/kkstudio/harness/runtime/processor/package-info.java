 ...
 * Gateway 打开回调门控晚于持久化 markRunning；外部 Handle.cancel 必须在 monitor 之外调用以避免与 Listener 回调死锁。
 *
 * <p>admission 前先用 Work-only 短事务验证 claim 真实 owned，再进 per-invocation guard 防止同一 claim 重复 / 并发投递误伤合法
