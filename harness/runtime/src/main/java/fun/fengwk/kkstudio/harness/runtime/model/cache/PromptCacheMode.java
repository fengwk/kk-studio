 ...
 *   <li>{@link #AUTOMATIC}：Provider 自行决定缓存命中；harness 不发送任何 cache hint，且需要 Provider 报告 cache 使用量。
 *   <li>{@link #AFFINITY}：harness 通过 Provider 资源 ID 和会话前导内容派生 cache key（如 {@code
 *       prompt_cache_key}）。 该 mode 不依赖显式 breakpoint。
