...
Optional first directive after Begin Patch (omit to use the daemon default workdir):
...
Update in a worktree without changing the daemon default workdir:
*** Begin Patch
...
- Paths are relative to `*** Workdir:` when present, otherwise the daemon default workdir. Keep all paths inside the daemon environment root.
...
- Every Update that contains hunks must add or remove at least one line; semantic no-ops are rejected. A Move may omit hunks for a pure rename.
