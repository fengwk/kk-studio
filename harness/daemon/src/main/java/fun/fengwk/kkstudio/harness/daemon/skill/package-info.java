 ...
 * .kkstudio-commit} marker 记录。tree 先拉取进 {@code skill-work/cache} 的 bare Git cache（origin URL
 * 与请求不一致时整份丢弃重建， 绝不复用其它仓库的对象），再逐路径物化进 {@code skill-work/staging} 的随机子目录并校验，最后以目录替换原子发布；
 * 任何一步失败都保留旧目录，并用进程内单包锁串行同一 Package 的并发安装。
 *
