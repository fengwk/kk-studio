 ...
 * 语义；它们不启动外部搜索命令。
 *
 * <p>稳定的能力集合：{@code read}、{@code write}、{@code edit}、{@code apply_patch}、{@code bash}、{@code
 * grep}、{@code find}、{@code lsp_goto_definition}、{@code lsp_workspace_symbols}、{@code
 * lsp_java_decompile}。{@code apply_patch} 只处理 invocation workspace 内的 UTF-8 文本，并在全部预检通过后提交变更。LSP
 * tools 使用可选的本机命令 bridge（{@code kkstudio.daemon.lsp-bridge}）；未配置时返回明确的不可用错误，但 {@code
