 ...
 *
 * <p>稳定的能力集合：{@code read}、{@code write}、{@code edit}、{@code apply_patch}、{@code bash}、{@code grep}、
 * {@code find}、{@code lsp_goto_definition}、{@code lsp_workspace_symbols}、{@code
 * lsp_java_decompile}。LSP tools 使用可选的本机命令 bridge（{@code
 * kkstudio.daemon.lsp-bridge}）；未配置时返回明确的不可用错误，但 {@code lsp_java_decompile} 对可解析的 class 目标可回退到
 * {@code javap}。
 *
