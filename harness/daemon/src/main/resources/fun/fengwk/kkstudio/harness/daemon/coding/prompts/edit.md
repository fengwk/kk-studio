...
- `read` text output starts with a small header (`path`, `ends_with_newline`, and `lsp`), then numbered body lines in `number|content` form.
...
- `workdir` (optional, default: the daemon's current working directory; if provided, resolve relative paths from that directory).
