 ...
 *
 * <p>{@code name} 是全局唯一 Skill 名；{@code packageName}/{@code packageVersion} 唯一确定不可变 package 版本；
 * {@code contentRevision} 精确锁定内容。四者共同保证 {@code load_skill} 取回的是选择当时的版本，而不是同名的新版本。
 *
 * @param name Skill 全局唯一短名，必填
 * @param packageName 承载该 Skill 的 package 名，必填
 * @param packageVersion 承载该 Skill 的不可变 package 版本，必填
