...
--
-- V1 默认行（id=1）是 write/edit/bash 各 `* -> ask`，read 保持不限制（生产默认）。e2e 验收要求 read 也进入审批，
-- 因此本 seed 把 tool.permission 覆盖为 read/write/edit/bash 各 `* -> ask`（defaultYolo 仍为 false）。这是 e2e 的
