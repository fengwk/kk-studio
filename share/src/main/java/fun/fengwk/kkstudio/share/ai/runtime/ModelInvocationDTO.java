...
public class ModelInvocationDTO {
  ...
  /** 所属 Turn 的 TURN_START Entry 主键：strict positive decimal string。 */
  private String turnStartEntryId;

  /** 调用创建时的 head Entry 主键（basis）：strict positive decimal string。 */
  ...
  /**
   * 结果挂载的 Entry 主键：strict positive decimal string；TURN_END 应用后非 null（{@code @JsonInclude(ALWAYS)}）。
   */
  @JsonInclude(JsonInclude.Include.ALWAYS)
