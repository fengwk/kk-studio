...
create table if not exists agent_model (
    ...
    description       varchar(512) null comment '描述',
    capabilities_json longtext null comment '模型能力 JSON',
    limit_json        longtext null comment '模型限制 JSON',
    pricing_json      longtext null comment '模型价格 JSON',
...
create table if not exists agent_definition (
    ...
    subagents_json        longtext null comment '子 agent 名称 JSON 数组',
    skills_json           longtext null comment '技能名称 JSON 数组',
    gmt_create            datetime(3) not null default current_timestamp(3) comment '创建时间',
    gmt_modified          datetime(3) not null default current_timestamp(3) on update current_timestamp(3) comment '更新时间',
    version               bigint not null default '0' comment '数据版本号',
    primary key (id),
...
create table if not exists agent_session_head (
    ...
    gmt_create     datetime(3) not null default current_timestamp(3) comment '创建时间',
    gmt_modified   datetime(3) not null default current_timestamp(3) comment '更新时间',
    version        bigint not null default '0' comment '数据版本号',
    primary key (id),
