 ...
 * MySQL/H2.
 *
 * <p>账本写入必须以 {@code unique(assistant_entry_id)} 兜底幂等，并以 thread/session 索引支撑查询；任何 0/2 行结果必须以 {@link
 * java.util.ConcurrentModificationException} 上抛，由事务边界统一回滚。本包不在此层引入任何事务/聚合/补算业务。
 */
