...
public interface ThreadReconcileMapper extends BaseMapper {
  ...
  @Insert(
      "insert into harness_model_invocation (id, thread_id, session_id, source_head_entry_id, execution_epoch, request, status, attempt, created_at) values (#{id}, #{threadId}, #{sessionId}, #{sourceHeadEntryId}, #{epoch}, cast(#{requestJson} as jsonb), 'QUEUED', 1, #{now})")
