...
public interface ThreadReconcileMapper extends BaseMapper {
  ...
  @Insert(
      "insert into harness_tool_invocation (id, thread_id, session_id, assistant_entry_id, model_invocation_id, ordinal, tool_call_id, descriptor, arguments, environment_name, execution_epoch, status, attempt, permission_state, yolo_enabled, created_at) values (#{id}, #{threadId}, #{sessionId}, #{assistantEntryId}, #{modelInvocationId}, #{ordinal}, #{toolCallId}, cast(#{descriptorJson} as jsonb), cast(#{argumentsJson} as jsonb), #{environmentName}, #{epoch}, 'QUEUED', 1, 'PENDING', #{yoloEnabled}, #{now})")
  int insertToolInvocation(
      @Param("id") long id,
      @Param("threadId") long threadId,
      @Param("sessionId") long sessionId,
      @Param("assistantEntryId") long assistantEntryId,
      @Param("modelInvocationId") long modelInvocationId,
