...
/**
 * Core 侧的 Harness 自动对话压缩配置现读通道：每次决策点从 {@link SystemSettingsSnapshot} 的 {@code
 * aiRuntime.compactionKeepRecentTokens} 与 {@code aiRuntime.compactionFallbackModel} 映射 {@link
 * CompactionConfig}。
 *
 * <p>{@link CompactionConfigProvider} 同时被 core 的 {@code DatabaseTurnResolver} 与 web 组合根的 {@code
 * ThreadProcessorConfig} 消费，因此放在 core 作为唯一定义点；web 不再重复声明。aiRuntime 配置 live 生效，无需重启。
 */
