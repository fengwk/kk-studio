...
/** {@code harness_session_blob_ref} 的原子 SQL 入口。 */
@Mapper
public interface SessionBlobRefMapper extends BaseMapper {

  @Insert(
      """
      insert into harness_session_blob_ref (session_id, blob_id)
  ...
  @Delete(
      "delete from harness_session_blob_ref where session_id = #{sessionId} and blob_id = #{blobId}")
  int delete(@Param("sessionId") UUID sessionId, @Param("blobId") UUID blobId);

  @Select(
      "select exists (select 1 from harness_session_blob_ref where session_id = #{sessionId} and blob_id = #{blobId})")
  boolean exists(@Param("sessionId") UUID sessionId, @Param("blobId") UUID blobId);

  @Select(
      "select blob_id from harness_session_blob_ref where session_id = #{sessionId} order by blob_id")
