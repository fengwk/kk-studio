 ...
 *
 * <p>本包不保存 durable state，不实现 subscriber、worker lifecycle、Web SSE、PostgreSQL recovery 或 legacy
 * store。Redis 仅作为 lossy wake hint 与有界 realtime projection 通道；调用方负责隔离失败，adapter 只做严格 deterministic
 * wire 编码与原生 Redis 操作。
 */
