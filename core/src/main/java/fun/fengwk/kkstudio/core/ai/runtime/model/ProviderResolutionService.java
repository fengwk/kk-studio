 ...
 *
 * <p>Implementations look up the immutable persisted Provider revision by {@code
 * request.model().providerName()} and {@code request.model().providerVersion()}, validate that the
 * persisted type matches the frozen {@code ProviderType} on the request, confirm a Harness {@code
 * ProviderFactory} is registered for that type and freeze a short-lived Provider adapter resource.
 * The adapter creates the SDK-bound {@link ModelProvider} only on the Provider I/O executor, after
 * the durable deadline has capped the transport timeout.
 */
