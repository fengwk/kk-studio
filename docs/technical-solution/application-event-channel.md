...
{"version":1,"type":"event","resource":{...},"name":"revision","cursor":"<canonical decimal>","data":{"revision":"<canonical decimal>"}}
...
- `subscribed`：订阅已在 wire 上建立（首次与每次重连重订阅后都会发送）；`cursor` 是订阅建立瞬间的 durable cursor——Thread 为 revision、Canvas 为 version。该 cursor 之后的事件保证送达，之前的由客户端随后拉取的 snapshot / `/changes` 覆盖。
- `event` 的 `name` 为 `revision` / `realtime` / `version`：
  - Thread `revision`：`data` 为 `{"revision":"N"}`，顶层 `cursor` 必带且与 `data.revision` 完全相等；
...
- 每个资源只有一组上游：Thread = revision source + realtime source，Canvas = version source；首个本地订阅建立上游，最后一个释放时关闭。重复 subscribe 在传输层幂等。
...
- revision/version 信号在投递前按 ack cursor 过滤掉不晚于游标的陈旧值。
...
- `subscribed` / `revision` / `resync` / 资源级 `error` 都触发 snapshot 对账，`realtime` 事件走 overlay reducer。
