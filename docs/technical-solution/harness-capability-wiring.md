...
RuntimeToolsConfiguration
  -> loadSkillTool / TaskTool（内部 PLATFORM Tool beans；TaskTool 位于 harness.runtime.subagent，依赖 HarnessThreadChangeSource，由 web 组合根适配 ThreadRevisionEventSource 提供）
...
- **`load_skill` 与 `task` 是两个 internal Platform Tool**（`ToolCatalog(descriptors, internalNames)` 的 internal 集合 = `{load_skill, task}` + 插件 INTERNAL 贡献），从 selectable 集合移出，不出现在 Agent 可选择目录中。`task` 由 `RuntimeToolsConfiguration` 装配（`harness.runtime.subagent.TaskTool` + live `SubagentConfigProvider` + `SubagentRunRegistry`），被 `activeTools` 显式选择后由 Resolver 用 `taskTool.descriptor()` 现拼绑定。
...
`ModelRequestSpec(providerType, model, variant, preambleMessages, toolBindings, skillBindings, subagentBindings, cacheControl)`（无 YOLO/contextWindow/history messages/Provider tools/compaction metadata 副本）：
...
- basis EntryPath 末尾存在 owned `TURN_START.compaction` 时，tool bindings、skill bindings、subagent bindings 必须全空；phase/trigger/executionModel 与 cut/prefix/history anchors 只冻结在该 TURN_START；
