...
实现 `acceptCommands`（NEW_SESSION / ENTRY / THREAD 单原语）、`findThreadCommand`、`stop`、`decideToolApproval`、`setThreadYolo`、`getThreadSnapshot`、`getSessionEntries`、`listThreadsBySession`；不提供 create/delete（Session/Thread 只允许在第一批 Command 被接受时创建，深删除由 Platform `SessionDeletionOrchestrator` 经 `HarnessStore.Transaction` 编排）。
