...
Model terminal apply 时，Reconciler 按 Provider tool call 与冻结 ToolBinding materialize ToolInvocation。ToolInvocation 保存 descriptor、type、arguments、Environment target、permission state 与 request 的 `yoloEnabled`。
