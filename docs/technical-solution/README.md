...
- Chat/Canvas 归属使用 `chat_session(session_id,chat_id)` / `canvas_session(session_id,canvas_id)` 真实 FK 关系表；不存在 `chat_thread`、`canvas_document.thread_id` 或 owner-to-Thread association。CanvasDocument 不含 threadId；Canvas 经 `canvas_session` 持有多个 Session。Canvas Graph version 初始 0，每个成功 graph command/function 状态前进 +1；Harness command acceptance 不前移 graph version。
