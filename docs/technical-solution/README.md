...
flowchart TD
    ...
    A --> L[environment-daemon-gateway.md<br/>Environment Daemon Gateway 与固定 ToolCatalog]
...
| 5 | [harness-storage-runtime.md](harness-storage-runtime.md) | PostgreSQL durable activation、Redis 实时流 | 实现持久化与事件驱动基础设施 |
...
| 8 | [frontend-implementation-design.md](frontend-implementation-design.md) | Chat 默认 Agent/Environment、统一 ToolCatalog、路由、状态管理与测试边界；视觉约定见 [前端设计规范](../product-design/frontend-design-system.md) | 修改前端页面或 API 适配 |
| 9 | [harness-capability-wiring.md](harness-capability-wiring.md) | Spring ObjectProvider、ProviderFactories、统一 ToolCatalog、ToolFactories、interceptor chain | 修改 Harness 能力装配 |
...
| 13 | [environment-daemon-gateway.md](environment-daemon-gateway.md) | Environment 注册、固定 ToolCatalog、Daemon v1 WebSocket、RemoteTool 分发 | 接入远端 Environment Tool |
...
| PostgreSQL durable activation、Redis realtime | [harness-storage-runtime.md](harness-storage-runtime.md) |
...
| Environment Daemon Gateway 与固定 ToolCatalog | [environment-daemon-gateway.md](environment-daemon-gateway.md) |
