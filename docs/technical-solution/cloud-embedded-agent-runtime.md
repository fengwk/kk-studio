...
Thread 保存 agent/model/yolo；完整 Agent 配置（prompt/tools/skills/subagents/policy/environmentName）在 Turn 时从当前 AgentDefinition 动态装载。tools/skills 使用短名；Skill 元数据 platform-first 解析后写入 runtime config，并渲染为 system prompt 的 `<available_skills>`。`SET_AGENT` / `SET_MODEL` / `SET_YOLO` 作为有序 Input 在消息边界 Harvest 后生效，不即时改写进行中的模型请求。
