...
Command 类型固定为 `USER_MESSAGE`、`CUSTOM_MESSAGE`、`SET_ENVIRONMENT`、`SET_AGENT`、`SET_MODEL`、`SET_ACTIVE_TOOLS`。SET 前缀顺序固定为 environment → agent → model → active tools；初始化 batch 以一条末尾 user-like message 结束；THREAD batch 是一条用户消息或一条 SYSTEM steering。YOLO 不进 mailbox，而由 `setThreadYolo` 直接控制。
...
`ToolInvocation` 保存冻结 `ToolCall`、`ToolBinding`、assistant Entry、ordinal、approval、result、effects 和 error。状态为：
