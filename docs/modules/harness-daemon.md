...
Environment Daemon 是独立进程，负责固定 Environment root 内的 coding tools、MCP bridge、Skill discovery、目录浏览和 Daemon v3 WebSocket wire。它只依赖 `harness-tool` 的 Tool SPI/wire contract，不依赖 Runtime、Infra、Platform、Spring、数据库或 Model/Agent。

[`DaemonMain`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/DaemonMain.java) 是进程入口；[`DaemonRuntime`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/DaemonRuntime.java) 管理连接、journal、工具执行、重连、超时和 shutdown。Daemon 的 invocation journal 是进程内 execution fact；WebSocket 只传递消息。
...
- 在统一 Environment root 内安全执行 coding tools，提供 MCP/Skill 的本地能力摘要和按需调用。
...
- 不持久化 Agent/Session/Invocation/Work，不实现 Runtime ToolProcessor 或权限决策。
...
### DaemonMain、config 与固定 tool registry
...
默认 heartbeat 15s、reconnect initial 1s、reconnect max 30s、default tool timeout 5min；environment root 默认启动用户 HOME 的 canonical existing directory；未显式指定 skill dir 时只在 `~/.agents/skills` 存在时纳入。`note` 只接受可信操作者配置或按 OS 生成的稳定默认文本，最多 512 字符单行。

[`DaemonToolRegistry`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/DaemonToolRegistry.java) 在 runtime 构造完成时 freeze，freeze 后不能注册新 Tool。生产 coding catalog 由 [`CodingTools`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/coding/CodingTools.java) 固定注册：
...
MCP 无论 server 是否配置都注册固定 `mcp_list_tools` 和 `mcp_call_tool`，保持 Daemon registry 与 Environment catalog 一致。
...
### Coding Tools 与路径/资源边界

所有 filesystem coding tools 通过 [`EnvironmentPathBoundary`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/coding/EnvironmentPathBoundary.java)：

- 默认 workdir 来自每个 `ToolExecutionRequest.workdir`，由 INVOKE 的 `workspacePath` canonicalize 后写入；
...
Resource store 默认为 Environment root 下 `.kkstudio/resources` 的 content-addressed local store；大/二进制结果转为 ResourceToolContent。结果由 `DaemonToolResultCodec` 编码：PARTIAL 只允许 text/json，COMPLETED 资源预检后才允许 store read/write；默认资源聚合 8 MiB、最终 payload 16 MiB、contents 64 项。
...
[`McpServerRegistry`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/mcp/McpServerRegistry.java) 为每个 server 独立初始化；单 server 失败记录 FAILED，不影响其它 server/coding/skills，启动后状态和 tool specs 冻结；close 对成功 client 恰好一次。错误摘要单行最多 500 字符，并剔除 headers/environment values、command 和 URL。完整 schema 不进 READY wire，固定 bridge tool 按需返回。

`LIST_DIRECTORY` 是 control plane，不进入 invocation journal 或 active tool slot；payload `requestId` 关联，单层列表最多 1000 项，稳定按名称排序，wire path 只使用 Environment root 下 canonical 相对路径，不回显 daemon 绝对路径。实现见 [`EnvironmentDirectoryBrowser`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/EnvironmentDirectoryBrowser.java) 和 [`DaemonDirectoryCodec`](../../harness/tool/src/main/java/fun/fengwk/kkstudio/harness/tool/daemon/DaemonDirectoryCodec.java)。
...
sequenceDiagram
  ...
  G->>D: INVOKE(v3, invocationId, workspacePath, args)
...
INVOKE 的 `workspacePath` 先通过共享 `EnvironmentWorkspacePath` 校验，再 `resolve + normalize + toRealPath + startsWith(root) + isDirectory`；失败、未知 tool 或 version mismatch 都是 deterministic FAILED，绝不发送 STARTED。timeout 由 scheduler 计时，terminal claim 后调用 Tool handle cancel；`CANCEL` 对 RUNNING invocation 发送 CANCELLED 并取消 handle，对 terminal invocation 重放 terminal。

Daemon disconnect 后保留 journal 和 running execution 的进程内事实，连接 generation 进入重连；gateway 重新发送相同 invocationId 时，RUNNING 重放 `STARTED(replayed=true)`，terminal 重放相同 terminal payload。Tool callback 的 toolCallId 不等于 invocationId、result 编码超限、Resource store 失败或 partial 包含 resource/binary 时确定性 FAILED。

## 不变量、failure / recovery

- `environmentName` 是唯一 wire scope；另一个 live daemon 持有相同名称时进入 FAILED，不重连。
- Tool registry 在 runtime 构造期 freeze；READY 后不变更本地 catalog、Skill metadata 或 MCP server/tool snapshot。
...
- STARTED 只在 workspace、tool name/version、Tool request 和 runtime state 均通过后发送；invalid request 不产生 STARTED。
- timeout/cancel/shutdown 使用 `RunningInvocation` 的 terminal AtomicBoolean，完成、失败、取消和 timeout 只选择一个 terminal。
- 断线不把 WebSocket 当事实源；journal 负责 duplicate replay，Tool 本地 execution 结束后仍按 terminal-once 处理。
...
- gateway 可通过 invocation timeout 覆盖 descriptor/default timeout，但 `0` 表示不覆盖，最终 deadline 始终有效。
...
- [`DaemonToolRegistry.java`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/DaemonToolRegistry.java)、[`CodingTools.java`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/coding/CodingTools.java)、[`EnvironmentPathBoundary.java`](../../harness/daemon/src/main/java/fun/fengwk/kkstudio/harness/daemon/coding/EnvironmentPathBoundary.java)
