...
- 使用纯黑与深灰中性色作为页面和编辑器基调，主要强调色为白与浅灰；成功状态可以克制地使用低饱和绿色。
...
- Stage 接近黑色，点阵网格和关系线保持低对比；Frame、节点与浮层用黑灰层级区分。
- 对象边框默认弱化，只在悬停、选中和错误时增强，选中态使用白或浅灰而非高饱和主色。
- 控制面按需出现：底部 Dock 固定但紧凑，添加菜单、消息 thread 和生成面板不激活时不占用画布。
