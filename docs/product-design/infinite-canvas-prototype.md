...
>
> 对象状态、上下文快照、Agent / Generator 执行、保存撤销和异常恢复规则见 [`infinite-canvas-product-logic.md`](infinite-canvas-product-logic.md)。
...
flowchart TD
    ...
    Editor --> GenerationNodes[文本、图片与视频生成节点及上下文操作台]
...
| 生成节点 | 在当前视口安全区承载固定类型的生成草稿、当前输出和运行状态，保存独立 Prompt、能力、参考素材和参数 |
| 节点上下文操作台 | 作为 Stage 的屏幕空间浮层跟随当前生成节点，优先位于节点下方并避让顶部、左右边界与 Dock |
...
| 生成节点 | 打开对应类型的上下文操作台、调整能力与参数、继续生成 |
| 文件 | 预览、提取、总结、拆分为卡片 |
...
Running <-> Paused
Running -> WaitingForInput -> Running
Running -> Failed
Running -> Cancelled
```
...
- Dock 的 `+` 菜单从 Dock 左侧上方向上展开，使用类似右键菜单的单列竖向排列，固定提供文本生成、图片生成、视频生成、文件和 Frame 五项。
- 三类生成入口在当前视口安全区创建对应生成节点，自动选中并打开节点上下文操作台。
...
| Generator | 创建时固定的文本/图片/视频类型、独立草稿与当前输出、上下文操作台和原地生成 |
...
- 从 Dock 的单列竖向添加菜单选择文本生成、图片生成或视频生成，在当前视口安全区创建对应生成节点并自动选中。
- 生成节点区分文本、图片和视频预览，以“当前输出、未提交修改、活跃或失败运行”组合表达状态；节点保持可拖动、可聚焦和可继续编辑。
- 选中生成节点时，在节点附近打开 Stage 直接承载的屏幕空间操作台。操作台随节点拖动、视口平移、缩放和窗口尺寸变化重新定位，优先放在节点下方，空间不足时放到上方，并避让左右、顶部和 Dock 安全区。
- 操作台包含节点标题与状态、当前固定类型的能力 tabs、参考素材、Prompt、参数 chips、成本、提交与关闭，不提供文本、图片、视频之间的类型切换。
- 生成节点类型在创建时确定并保持固定；每个节点独立保存草稿配置、当前输出和 Generation Run 引用，切换节点时恢复对应配置。
- 生成成功后原地更新当前输出，刷新预览、标题、摘要和参数；旧输出保留在运行历史中，操作台继续关联当前节点，thread 只追加完成记录且不自动展开。
...
- 新建画布流程。
- 基础平移、缩放、框选、拖拽和多选。
- Text、Image、File、Frame、Generator、Agent Run、Result Group。
- 底部 Agent Dock、向上展开的单列五项添加菜单、按需消息 thread、固定类型生成节点与节点上下文操作台。
- 选区/整图上下文切换与选区上下文工具条。
