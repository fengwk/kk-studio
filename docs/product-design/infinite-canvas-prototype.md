...
| 区域 | 职责 |
| --- | --- |
| 顶栏 | 返回、标题、保存状态、分享、导出和低频菜单 |
| 左侧工具条 | 选择、平移、文本、添加、Frame、命令入口 |
| 中央画布 | 内容编辑、空间组织和运行结果落位 |
| 右侧 Copilot | 选区/整图上下文、自然语言任务、运行列表和高级参数 |
| 右下角 | 缩放、适应视图、小地图 |

右侧 Copilot 默认折叠。用户执行 AI 动作、查看运行详情或主动打开时展开，避免长期占据画布空间。
...
- 背景使用接近现有 `--bg` 的深色，网格对比度低于内容对象。
- 对象边框默认弱化，只在悬停、选中和错误时增强。
- 工具条无大面积胶囊容器和重阴影。
- 浮层只在相关上下文出现。
- 图片和视频是视觉主体，控制按钮不长期覆盖媒体。
- Agent Run 使用明确但克制的状态色，运行中不使用持续大面积动画。

### 10.3 避免的形态

- 永久占据底部的大型工具 Dock。
- 将所有节点类型、模型和生成参数平铺为十几个图标。
- 每次生成都创建可见连线，导致关系网遮挡内容。
- 把聊天侧栏作为唯一 AI 入口。
...
| 路线 | 当前判断 |
| --- | --- |
| tldraw SDK | 产品级白板体验强，但当前版本使用自定义 SDK 许可，生产授权与水印规则需进一步核验；其 shape/whiteboard 模型也不是本方案富业务节点的最短路径 |
...
| 直接复用本地 `infinite-canvas` | 项目为 AGPL-3.0，修改后通过网络向用户提供服务时可能承担对应源代码提供义务；适合作为行为和模块边界参考，不直接复制源码 |
...
| Miro AI | 整张画布作为 Prompt、Agent 在画布上构建结构、流程模板化 | Canvas Copilot 必须同时支持选区和整图上下文 |
...
6. 右侧 Copilot 折叠后，核心画布区域不被固定面板侵占。
7. 基础指针操作符合主流设计工具习惯。
