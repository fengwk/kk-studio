...
| 生成节点 | 在当前视口安全区承载文本、图片或视频生成草稿与结果，保存独立 Prompt、能力、参考素材和参数状态 |
...
| 生成节点 | 打开节点上下文操作台、转换文本/图片/视频类型、继续生成 |
| 文件 | 预览、提取、总结、拆分为卡片 |
...
- Dock 的 `+` 菜单从 Dock 上方向上展开，固定提供文本生成、图片生成、视频生成、文件和 Frame 五项。
...
| Generator | 文本/图片/视频模式、节点独立草稿、上下文操作台和原地生成 |
...
- 从 Dock 的向上添加菜单选择文本生成、图片生成或视频生成，在当前视口安全区创建对应生成节点并自动选中。
- 生成节点区分文本、图片和视频预览，并以 `draft / generated` 状态表达配置中与已完成；节点保持可拖动、可聚焦和可继续编辑。
- 选中生成节点时，在节点附近打开 Stage 直接承载的屏幕空间操作台。操作台随节点拖动、视口平移、缩放和窗口尺寸变化重新定位，优先放在节点下方，空间不足时放到上方，并避让左右、顶部和 Dock 安全区。
- 操作台包含节点标题与状态、三类模式切换、模式能力 tabs、参考素材、Prompt、参数 chips、成本、提交与关闭；每个模式拥有独立默认 Prompt、能力和参数集合。
- 每个生成节点独立保存模式、Prompt、参数索引、参考素材、能力和状态。切换节点恢复对应状态，切换模式转换当前节点并同步尺寸、标题、预览与默认配置。
- 提交后原地把当前节点更新为 `generated`，刷新预览、标题、摘要和参数；操作台继续关联当前节点，thread 只追加完成记录且不自动展开。
...
- 底部 Agent Dock、向上展开的五项添加菜单、按需消息 thread、生成节点与节点上下文操作台。
- 选区/整图上下文切换与选区上下文工具条。
